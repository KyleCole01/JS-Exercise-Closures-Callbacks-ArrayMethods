...
function processDuplicateFree(list, callback) {
 const returnset = new Set(list);
 const returnArray = [...returnset]
...
function getFullNames(runners) {
  var newArray = runners.map((runner) => {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  var newArray = runners.filter((runner) => {
...
for(let i=0; i<runners.length;i++){
  tally = tally + runners[i].donation;
}
return tally;
}
